...
class Common_model extends CI_Model{
     ...
     * @return int 或 array
     */
    public function pageDate($table = '', $param = array(), $type = 'total', $show_field = '*', $order = array()){
        if (empty($table)){
            return FALSE;
        }
        
        $limit = (int)$param['limit'];
        $page  = (int)$param['page'];//开始查询的ID，在CI分页中
        ...
        if (!empty($order) && is_array($order)){
            ...
            if (!empty($primary_arr)){
                $this->db->order_by($primary_arr[0]->COLIMN_NAME,'DESC');
            }
        }
    ...
    public function update($table ='' ,$primary='',$id = 0, $param = array()) {
        ...
        $id = (int)$id;
        $this->db->where($primary,$id)->limit(1)->update($table,$param);
    ...
    public function delete($table = '', $primary = '', $id = array()){
        ...
        $this->db->where_in($primary, $id)->delete($table);
     ...
     */
    public function search($table ='', $param ='', $limit=NULL, $offset=NULL, $show_field = '*', $order = array()) {
        if (empty($table)){
            return FALSE;
        }
    ...
    public function getPrimary($database = '', $table = ''){
        ...
        $this->db->select('c.COLUMN_NAME')
                 ->form('INFORMATION_SCHEMA.TABLE_CONSTRAINTS AS t')
